...
SSHDeck 是一个面向 Windows 的可视化 SSH 管理工具。从 v0.2.0 起以**主机**为中心：每台 SSH 服务器是一个一级目录，其下挂载多条端口转发（二级目录），并支持发送指令、打开终端、上传密钥等操作。
...
- **远程连接与端口转发独立页面**：两页均按主机分组。「远程连接」展开主机后显示历史远端路径；「端口转发」展开主机后显示当前转发配置与管理操作。
- **网页打开**：运行中的端口转发可一键用系统默认浏览器打开其监听端口（`http://<监听地址>:<本机端口>`，`0.0.0.0` 按 `127.0.0.1` 处理）；主页与端口转发页的端口条都有，最适合 Local 转发。
- **发送指令**：通过 SSH 在主机上执行一条指令，输出显示在弹窗中；支持免密或一次性密码两种方式。
- **打开终端**：弹出一个已通过 SSH 连上服务器的外部 PowerShell 窗口，支持 Tab 自动补全。
- **通过 VS Code 打开**：「远程连接」页读取 VS Code Remote-SSH 历史（优先取扩展维护的最新记录），按主机 IP 匹配远端文件夹，并可用 VS Code 或终端重新打开。也可直接连接或手动输入远端路径；`~` 与相对路径会按远端家目录解析。程序会自动检测 VS Code 与 Remote-SSH 扩展。
...
- 主页是轻量监控板：当前连接按主机分组展示，每条转发可单独断开，并有「新建」快捷入口；置顶主机可快速进入「远程连接」，完整日志在独立的「日志」页。
...
- **应用内更新**：在「设置 → 软件更新」中检查 GitHub Releases，检测到更新的已签名版本后可原地安装并自动重启，无需手动下载安装包。开启「自动更新」后，启动时发现新版本会自动安装；关闭时则在主界面显示一条更新提示。

## 下载与运行

从 GitHub Releases 下载 `v0.3.0-beta.4`：

- 推荐下载安装包：`SSHDeck_0.3.0-beta.4_x64-setup.exe`
- 免安装压缩包：`SSHDeck_0.3.0-beta.4_x64-portable.zip`，解压后直接运行 `SSHDeck.exe`。

免安装版的设置与主机数据仍保存在 `%LOCALAPPDATA%\codex\ssh-port-forwarder\data`，不会写在程序旁边；系统需要 Windows OpenSSH Client 和 Microsoft Edge WebView2 Runtime。如需保持免安装方式，请手动下载新版 ZIP，应用内更新沿用安装版更新流程。
